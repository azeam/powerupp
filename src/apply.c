...
void on_btn_apply_clicked(GtkButton *button, app_widgets *app_wdgts)
{
  ...
  if (readerror == 0) {
    snprintf(applysetup, sizeof applysetup, "pkexec bash -c \"chmod 666 /sys/class/drm/card%d/device/pp_table;sudo -u %s %s %s %s;chmod 644 /sys/class/drm/card%d/device/pp_table;%s\"", card_num, username, "python3", upppath, writecmd, card_num, powersysfswrite);
